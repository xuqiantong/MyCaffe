...
print('Done in %.2f s' % (time.time()-start))
#temp = 0
record_result = open('result_record.txt','w')
...
print('using YanNet-%s, mean average precision is %.2f' % (args.feat, mean_average_precision))
print('mean query time is %.2f s' % mean_query_time)

# save precision@k
#with open('result/yannet_google_color_new_all_precision_at_k.txt', 'w') as f:
#    for i in range(args.nn_number):
#        f.write('%f\n' % (precsion_at_k[i]/query_num))

#with open('result/yannet_color_new_all_google_map.txt', 'w') as f:
